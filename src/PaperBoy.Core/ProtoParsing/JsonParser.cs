...
public partial class JsonParser
{
    ...
    private readonly JsonFormatter _protoJsonFormatter = new(
    ...
    /// <summary>
    ///     Reads bytes from proto descriptor generated by protoc-util. Support raw descriptor file. CLR classes not required.
    /// </summary>
    /// <param name="protoDescriptor">Proto descriptor in bytes.</param>
    /// <returns>JSON-like string represented original proto with stubbed values.</returns>
    ...
    /// <summary>
    ///     Using CLR class IMessage to generate JSON with data stubs.
    /// </summary>
    /// <param name="dynamicProtoMessage">IMessage generated from Proto.</param>
    /// <returns>Human-readable JSON from CLR type with data stubs.</returns>
    internal string SerializeProtoFromClrMessage(IMessage dynamicProtoMessage)
    {
        var instance = (IMessage)Activator.CreateInstance(dynamicProtoMessage.Descriptor.ClrType);

        foreach (FieldDescriptor field in dynamicProtoMessage.Descriptor.Fields.InDeclarationOrder())
        {
            object value = CreateDefaultValue(field, 0);

            if (field.IsRepeated)
            {
                var list = (IList)field.Accessor.GetValue(instance);
                list.Add(value);
            }
            else
            {
        ...
        return _protoJsonFormatter.Format(instance);
    }

    /// <summary>
    ///     Parse raw descriptor file by default protobuf JsonFormatter.
    /// </summary>
    /// <param name="fileDescriptorSet"></param>
    /// <returns>Human-readable JSON with default values.</returns>
    internal string ParseWithDefaultValues(FileDescriptorSet fileDescriptorSet)
    {
        return _protoJsonFormatter.Format(fileDescriptorSet);
    }
